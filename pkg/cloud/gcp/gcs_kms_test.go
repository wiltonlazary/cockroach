...
var gcsKMSTestSettings *cluster.Settings

func init() {
	gcsKMSTestSettings = cluster.MakeTestingClusterSettings()
}
...
func TestEncryptDecryptGCS(t *testing.T) {
	...
	expect := map[string]string{
		...
		"GOOGLE_APPLICATION_CREDENTIALS": GoogleApplicationCredentialsParam,
	}
	...
	for _, id := range []string{"GCS_KEY_ID", "GCS_KEY_NAME"} {
		...
		t.Run(fmt.Sprintf("auth-empty-no-cred-%s", id), func(t *testing.T) {
			...
			uri := fmt.Sprintf("gcs:///%s?%s", keyID, params.Encode())
		...
		t.Run(fmt.Sprintf("auth-specified-%s", id), func(t *testing.T) {
			...
			uri := fmt.Sprintf("gcs:///%s?%s", keyID, q.Encode())
